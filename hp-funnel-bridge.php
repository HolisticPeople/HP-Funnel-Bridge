...
define('HP_FB_PLUGIN_VERSION', '0.2.27');
...
add_action('template_redirect', function () {
	...
	echo '<div id="element"></div><div style="margin-top:12px;"><button id="pay" disabled>Pay</button></div><div id="messages"></div></div><script>(async function(){try{var pub='.$pub_json.';var cs='.$cs_json.';var ret='.$here_json.';var rawSucc=(new URL(window.location.href)).searchParams.get(\"succ\")||\"\";var succ=rawSucc;try{succ=decodeURIComponent(succ);}catch(e){}try{succ=decodeURIComponent(succ);}catch(e){}var stripe=Stripe(pub);var elements=stripe.elements({clientSecret:cs});var paymentElement;var piId=\"\";try{paymentElement=elements.create(\"payment\");paymentElement.mount(\"#element\");}catch(pe){var m=document.getElementById(\"messages\");if(m)m.textContent=(pe&&pe.message)?pe.message:\"Failed to load payment fields\";return;}var btn=document.getElementById(\"pay\");var msg=document.getElementById(\"messages\");var amt=document.getElementById(\"amount\");function bindCopy(){var nodes=document.querySelectorAll(\".copy\");nodes.forEach(function(n){n.addEventListener(\"click\",function(){var sel=n.getAttribute(\"data-copy\");var el=document.querySelector(sel);if(!el) return;var txt=el.textContent||\"\";if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(txt);}else{var ta=document.createElement(\"textarea\");ta.value=txt;document.body.appendChild(ta);ta.select();try{document.execCommand(\"copy\");}catch(e){}document.body.removeChild(ta);}});});}bindCopy();try{var pir=await stripe.retrievePaymentIntent(cs);if(pir&&pir.paymentIntent){var cents=pir.paymentIntent.amount||0;var cur=(pir.paymentIntent.currency||\"usd\").toUpperCase();amt.textContent=\"Amount: $\"+(cents/100).toFixed(2)+\" \"+cur;piId=pir.paymentIntent.id||\"\";}}catch(e){}function isSafe(u){try{var x=new URL(u);return x.protocol===\"https:\"||x.protocol===\"http:\";}catch(_){return false;}}async function tryRedirect(){if(!succ||!isSafe(succ)||!piId){return;}msg.textContent=\"Payment processed. Finishing up...\";for(var i=0;i<30;i++){try{var r=await fetch(\"/wp-json/hp-funnel/v1/orders/resolve?pi_id=\"+encodeURIComponent(piId),{headers:{Accept:\"application/json\"}});if(r.ok){var t=await r.json();if(t&&t.order_id){var uu=new URL(succ);if(uu.search){uu.search+=\"&order_id=\"+encodeURIComponent(String(t.order_id));}else{uu.search=\"?order_id=\"+encodeURIComponent(String(t.order_id));}window.location.replace(uu.toString());return;}}}catch(e){}await new Promise(function(res){setTimeout(res,1000);});}/* fallback: redirect to base with hash so SPA can self-route without 404 */try{var baseUrl=new URL(succ);if(!baseUrl.hash||baseUrl.hash===\"#\"||baseUrl.hash===\"\"){baseUrl.hash=\"#upsell\";}if(baseUrl.search){baseUrl.search+=\"&pi_id=\"+encodeURIComponent(piId);}else{baseUrl.search=\"?pi_id=\"+encodeURIComponent(piId);}window.location.replace(baseUrl.toString());}catch(__){var u2=new URL(ret);u2.hash=\"#upsell\";if(u2.search){u2.search+=\"&pi_id=\"+encodeURIComponent(piId);}else{u2.search=\"?pi_id=\"+encodeURIComponent(piId);}window.location.replace(u2.toString());}}paymentElement.on(\"change\",function(e){btn.disabled=!e.complete;msg.textContent=\"\";});btn.addEventListener(\"click\",async function(){btn.disabled=true;msg.textContent=\"Processing...\";try{var submitRes=await elements.submit();if(submitRes&&submitRes.error){msg.textContent=submitRes.error.message||\"Please check your details\";btn.disabled=false;return;}var res=await stripe.confirmPayment({elements:elements,clientSecret:cs,confirmParams:{return_url:ret},redirect:\"if_required\"});if(res.error){msg.textContent=(res.error&&res.error.message)?res.error.message:\"Payment failed\";btn.disabled=false;}else{await tryRedirect();btn.disabled=true;}}catch(err){msg.textContent=(err&&err.message)?err.message:\"Payment failed\";}});}catch(_err){var m=document.getElementById(\"messages\");if(m)m.textContent=\"Failed to initialize payment\";}})();</script></body></html>';
	exit;
});

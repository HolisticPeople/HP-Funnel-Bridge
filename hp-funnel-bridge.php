...
define('HP_FB_PLUGIN_VERSION', '0.1.7');
...
add_action('template_redirect', function () {
	...
	echo '<div id="element"></div><div style="margin-top:12px;"><button id="pay" disabled>Pay</button></div><div id="messages"></div></div><script>(async function(){try{var stripe=Stripe("'.$pub.'");var elements=stripe.elements({clientSecret:"'.$cs_js.'"});var paymentElement=elements.create("payment");paymentElement.mount("#element");var btn=document.getElementById("pay");var msg=document.getElementById("messages");var amt=document.getElementById("amount");function bindCopy(){var nodes=document.querySelectorAll(\".copy\");nodes.forEach(function(n){n.addEventListener(\"click\",function(){var sel=n.getAttribute(\"data-copy\");var el=document.querySelector(sel);if(!el) return;var txt=el.textContent||\"\";navigator.clipboard&&navigator.clipboard.writeText?navigator.clipboard.writeText(txt):document.execCommand(\"copy\",true,txt);});});}bindCopy();try{var pi=await stripe.retrievePaymentIntent(\"'.$cs_js.'\");if(pi&&pi.paymentIntent){var cents=pi.paymentIntent.amount||0;var cur=pi.paymentIntent.currency||\"usd\";amt.textContent=\"Amount: $\"+(cents/100).toFixed(2)+\" \"+cur.toUpperCase();}}catch(e){}paymentElement.on(\"change\",function(e){btn.disabled=!e.complete;msg.textContent=\"\";});btn.addEventListener(\"click\",async function(){btn.disabled=true;msg.textContent=\"Processing...\";try{var submitRes=await elements.submit();if(submitRes&&submitRes.error){msg.textContent=submitRes.error.message||\"Please check your details\";btn.disabled=false;return;}var res=await stripe.confirmPayment({elements:elements,clientSecret:\"'.$cs_js.'\",confirmParams:{return_url:\"'.$here.'\"},redirect:\"if_required\"});if(res.error){msg.textContent=(res.error&&res.error.message)?res.error.message:\"Payment failed\";btn.disabled=false;}else{msg.textContent=\"Payment processed. You can close this page.\";btn.disabled=true;}}catch(err){msg.textContent=(err&&err.message)?err.message:\"Payment failed\";btn.disabled=false;}});}catch(_err){var m=document.getElementById(\"messages\");if(m)m.textContent=\"Failed to initialize payment\";}})();</script></body></html>';
	exit;
});

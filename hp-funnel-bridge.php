...
define('HP_FB_PLUGIN_VERSION', '0.1.3');
...
add_action('template_redirect', function () {
	...
	echo '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>HP Funnel Payment</title><style>body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;margin:24px;}#checkout{max-width:520px;margin:0 auto;}button{padding:10px 14px;border:1px solid #ccc;border-radius:6px;background:#111;color:#fff;cursor:pointer;}#messages{margin-top:12px;color:#c00}#amount{margin:6px 0 14px;color:#111;font-weight:600}</style><script src="https://js.stripe.com/v3/"></script></head><body><div id="checkout"><h2>Complete Payment</h2><div id="amount"></div><div id="element"></div><div style="margin-top:12px;"><button id="pay" disabled>Pay</button></div><div id="messages"></div></div><script>(async function(){const stripe=Stripe("'.$pub.'");const elements=stripe.elements({clientSecret:"'.$cs_js.'"});const paymentElement=elements.create("payment");paymentElement.mount("#element");const btn=document.getElementById("pay");const msg=document.getElementById("messages");const amt=document.getElementById("amount");try{const pi=await stripe.retrievePaymentIntent("'.$cs_js.'");if(pi&&pi.paymentIntent){const cents=pi.paymentIntent.amount||0;const cur=pi.paymentIntent.currency||"usd";amt.textContent=\"Amount: $\"+(cents/100).toFixed(2).toString().replace(/\\B(?=(\\d{3})+(?!\\d))/g,',')+\" \"+cur.toUpperCase();}}catch(e){}paymentElement.on(\"change\",(e)=>{btn.disabled=!e.complete;msg.textContent=\"\";});btn.addEventListener(\"click\",async()=>{btn.disabled=true;msg.textContent=\"Processing...\";try{const res=await stripe.confirmPayment({elements,clientSecret:\"'.$cs_js.'\",confirmParams:{return_url:\"'.$here.'\"},redirect:\"if_required\"});if(res.error){msg.textContent=res.error.message||\"Payment failed\";btn.disabled=false;}else{msg.textContent=\"Payment processed. You can close this page.\";btn.disabled=true;}}catch(err){msg.textContent=err&&err.message?err.message:\"Payment failed\";btn.disabled=false;}})})();</script></body></html>';
	exit;
});
